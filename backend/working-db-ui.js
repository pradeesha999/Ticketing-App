...
<body>
    ...
    <script>
            ...
            fetch('/api/stats')
                ...
                .then(stats => {
                    ...
                    for (const [collection, count] of Object.entries(stats)) {
                        statsHtml += '<div class="stat-card"><h3>' + collection + '</h3><div class="stat-number">' + count + '</div></div>';
                    }
            ...
            fetch('/api/collections')
                ...
                .then(collections => {
                    ...
                    for (const [name, data] of Object.entries(collections)) {
                        collectionsHtml += '<div class="collection-card">' +
        ...
        function viewCollection(collectionName) {
            fetch('/api/collection/' + collectionName)
                ...
                .then(data => {
                    ...
                    content.innerHTML = '<div class="json-viewer">' + JSON.stringify(data, null, 2) + '</div>';
                })
                .catch(error => {
                    const content = document.getElementById('content-' + collectionName);
        ...
        function viewSample(collectionName) {
            fetch('/api/collection/' + collectionName + '?limit=3')
                .then(response => response.json())
                .then(data => {
                    const content = document.getElementById('content-' + collectionName);
                    content.innerHTML = '<div class="json-viewer">' + JSON.stringify(data, null, 2) + '</div>';
